...
<title>新浪微博V2接口演示程序-Powered by Sina App Engine</title>
    <script tppe="text/javascript">
        ...
        function show(id)
        {
                //thisform.agreesubmit.disabled = false;
              // var x=document.getElementsByName("form"+id);
              // alert(x[0].children);
               // alert(thisform.agreesubmit.disabled);
               //disagreeform.disagreesubmit.disabled=false;
...
if( isset($_REQUEST['text']) ) {
	$ret = $c->update( $_REQUEST['text'].'#求真相#');	//发送微博
...
<div style="padding:10px;margin:5px;border:1px solid #ccc">
    ...
    ?>
   <!-- <form  name="form<?// echo $id++;?>"> -->
        <!--<input type="submit" value = "发起投票" onclick="show(<? //echo $id;?>)">-->
        <form style="display:inline;" action="handleagree.php"> 
            <input type="hidden" name ="pid" value="<?echo $item['idstr'];?>"> 
        ...
        <form style="display:inline;" action="handledisagree.php">
            <input type="hidden" name ="pid" value="<?echo $item['idstr'];?>"> 
            <input type="hidden" name ="uid" value="<?echo $item['user']['idstr'];?>"> 
            <input type="hidden" name ="agree" value="0"> 
            <input type="hidden" name ="disagree" value="0"> 
